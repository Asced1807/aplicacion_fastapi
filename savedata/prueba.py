...
@app.post("/enviar/")
async def submit_formulario(request: Request, nombre_completo: str, correo_electronico: str, sexo: int, fecha_nacimiento: date, provincia: str, ubicacion_especifica: str, tipo_recurso: str, cantidad: int, unidad: str, fecha_extraccion: date, db:Session=Depends(database)):
    ...
    Usuario = DatosFormulario(nombre_completo=nombre_completo, correo_electronico=correo_electronico, sexo=sexo, fecha_nacimiento=fecha_nacimiento, edad=edad_calculada, provincia=provincia, ubicacion_especifica=ubicacion_especifica, tipo_recurso=tipo_recurso, cantidad=cantidad, cantidad_calculada=cantidad_kilogramos ,unidad=unidad, fecha_extraccion=fecha_extraccion)
    db.add(Usuario)
    db.commit()
    db.refresh(Usuario)
    db.close()
